...
const WithCreateClientActionsProvider = ({ children }: { children: ReactElement }) => {
	...
	const renderElements = [
		{
			...
			modalActionsElement: [
				<Button key={1} onClick={onNextStep}>
					Continue
				</Button>,
			],
		...
		},
		{
			...
			modalActionsElement: [
				<Button key={1} onClick={onPreviousStep} variant='secondary' icon={<WestIcon />}>
					Back
				</Button>,
				<Button key={2} onClick={onSaveClient}>
					Save
				</Button>,
			],
			onClose: () => setShowModal(false),
	...
	useEffect(() => {
		if (activeStep !== undefined) {
			setModal(<StepModals steps={STEPS} data={renderElements[activeStep]} />);
		}
